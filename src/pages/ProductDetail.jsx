...
const ProductDetail = () => {
  ...
  // ✅ Modal navigation hooks (always run, but inactive if no modal)
  useEffect(() => {
  ...
  // ✅ Prepare derived data safely (even when loading)
  const images =
  ...
  const isInStock = (product?.stockStatus || "")
    .toLowerCase()
    .includes("in stock");
  const totalWeight = minUnitValue * quantity;
  const totalPrice = (product?.currentPrice || 0) * quantity;
            ...
            <div className="space-y-4 border-t border-gray-200 dark:border-gray-700 pt-6">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  disabled={!isInStock}
                  className={`flex-1 flex items-center justify-center gap-2 px-6 py-4 rounded-lg font-semibold shadow-sm transition-all ${
                    !isInStock
